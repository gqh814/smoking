...
class DynamicModel:
    ...
    def transition_S(self, S, b):
        ...
        return np.clip(new_S, self.S_grid[0], self.S_grid[-1])
    ...
    def transition_lambda(self, lambda_val, S, sigma_lagged, sigma_current, t):
        ...
        log_ratio_sigma_1 = np.log(pi_S / pi_0)
        log_ratio_sigma_0 = np.log((1 - pi_S) / (1 - pi_0))

        # Apply vectorized conditions
        lambda_val = np.where(
            sigma_lagged == 0,
            np.where(sigma_current == 1, lambda_val + log_ratio_sigma_1, lambda_val + log_ratio_sigma_0),
            lambda_val
        )
    ...
    def prob_sigma_next_is_one(self, sigma_current, lambda_val, S):
        # print('fuck')
        # Case where sigma_current == 1, return 1.0
        result = np.where(sigma_current == 1, 1.0, 0)

        # Compute num where sigma_current != 1
        num = self.pi_of_S(S) * np.exp(lambda_val) + np.where(S != 0, self.pi_of_S(0), 0)
        
        # Compute final result where sigma_current != 1
        result = np.where(sigma_current == 1, 1.0, num / (1.0 + np.exp(lambda_val)))
        return result
    ...
    def bi_vec(self):
        ...
        for b in self.b_choices:
            u = self.utility(S_mesh, sigma_mesh, b)
            assert (u>0).all(), "Utility should be positive"
            better_choice = u > self.V[-1]
            self.V[-1] = np.where(better_choice, u, self.V[-1])
            self.Policy[-1] = np.where(better_choice, b, self.Policy[-1])

        # Earlier periods
        for t in reversed(range(self.T - 1)):
            for b in self.b_choices:
                u = self.utility(S_mesh, sigma_mesh, b)
                S_next = self.transition_S(S_mesh, b)
                i_next = np.argmin(np.abs(self.S_grid - S_next),axis=0)
                print(i_next)
                p1 = self.prob_sigma_next_is_one(sigma_mesh, lambda_mesh, S_mesh)
                lambda_next_1 = self.transition_lambda(lambda_mesh, S_mesh, sigma_mesh, 1, t)
                lambda_next_0 = self.transition_lambda(lambda_mesh, S_mesh, sigma_mesh, 0, t)
                k_next_1 = np.argmin(np.abs(self.lambda_grid - lambda_next_1),axis=2)
                k_next_0 = np.argmin(np.abs(self.lambda_grid - lambda_next_0),axis=2)
                cont_val = (p1 * self.V[t + 1, i_next, 1, k_next_1] +
                            (1 - p1) * self.V[t + 1, i_next, 0, k_next_0])
                total_value = u + self.beta * cont_val
                
                better_choice = total_value > self.V[t]
                self.V[t] = np.where(better_choice, total_value, self.V[t])
                self.Policy[t] = np.where(better_choice, b, self.Policy[t])
